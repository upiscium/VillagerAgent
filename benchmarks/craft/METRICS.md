...
- Oracle-aware Clarify hard rules are opt-in via `dual_dag.gated_clarification.oracle_aware_rules.enabled`. In oracle-assisted runs they can suppress Clarify for a single executable oracle candidate or for sufficiently separated top candidates, while preserving execution-blocker clarifications such as missing large-block span representation.
