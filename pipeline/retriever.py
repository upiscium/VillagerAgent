...
class Retriever:
    ...
    embeddings = OpenAIEmbeddings(
        model=os.environ.get("OPENAI_EMBEDDING_MODEL", OLLAMA_MODEL),
        ...
        def search(x):
            if isinstance(x, dict):
                for key, value in x.items():
                    ...
                    similarity_key = self.similarity_get(
                        query, key, embedding_query, embedding_key
                    )
                    similarity_value = self.similarity_get(
                        query, value_str, embedding_query, embedding_value
                    )
                    if similarity_key > threshold or similarity_value > threshold:
                        results.append(
                            (max(similarity_key, similarity_value), key, value)
                        )
                    if isinstance(value, (dict, list)):
                        search(value)
            elif isinstance(x, list):
    ...
    def flatten_search(self, query, data, threshold, max_results):
        ...
        for key, value in data_dict.items():
            ...
            similarity_value = self.similarity_get(
                query, value, embedding_query, embedding_value
            )
            if similarity_key > threshold:
                results.append(
                    (max(similarity_key, similarity_value), key, document2string(data))
                )
            elif similarity_value > threshold:
                results.append(
                    (
                        max(similarity_key, similarity_value),
                        value,
                        document2string(data),
                    )
                )
...
if __name__ == "__main__":
    ...
    test_data = {
        ...
        "address": {"country": "China", "province": "Shanghai", "city": "Shanghai"},
        "friends": [
            {
                "name": "Amy",
                "age": 18,
